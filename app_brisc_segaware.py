...
# - Adds:
#     * ROI-assisted ensemble (crop around tumor mask)
...
ALT_MIN        = 0.85   # alt prob must be at least this to flip
GAP_MIN        = 0.20   # alt - raw must exceed this gap to flip
...
def _region_supports(label, bbox, H, W):
    ...
    # for glioma/no_tumor we don't require a region pattern
    return True
    ...
    if _cls_model is None:
        if not os.path.exists(BRISC_CLS_PT):
            raise FileNotFoundError(
                ...
                "Run train_brisc_resnet.py (done) / export_models.py first."
            )
...
def classify_brisc_segaware(img: Image.Image):
    ...
    if seg_model is not None:
        try:
            ...
            dbg_lines.append(f"Segmentation error: {e}")
            seg_mask_224 = None
            tumor_frac = None
    else:
    ...
    if seg_mask_224 is not None and seg_mask_224.sum() > 0:
        try:
            ow, oh = orig.size
            seg_up = Image.fromarray(seg_mask_224 * 255, mode="L").resize(
                (ow, oh), resample=Image.NEAREST
            )
            seg_np = np.array(seg_up)
            bbox = _largest_cc_bbox(seg_np, pad_frac=0.10)
            if bbox is not None:
                roi_bbox = bbox  # (x1,y1,x2,y2) in original image coords
                x1, y1, x2, y2 = bbox
                ...
                if crop.size > 0:
                    ...
                    crop_masked[crop_mask == 0] = (
                        crop_masked[crop_mask == 0] * 0.25
                    ).astype(crop_masked.dtype)
                    ...
                    dbg_lines.append("ROI ensemble applied (alpha=0.6) on tumor classes only.")
        except Exception as e:
            dbg_lines.append(f"ROI ensemble error: {e}")
    ...
    # -----------------------------
    # NEW: conservative seg-aware flips between tumor classes
    # -----------------------------
    H, W = np.array(orig).shape[:2]
    # top-1 / top-2 from current probs
    order = np.argsort(probs)[::-1]
    top1_idx = int(order[0]); top2_idx = int(order[1])
    top1_lbl = LABELS[top1_idx]; top2_lbl = LABELS[top2_idx]
    top1_p = float(probs[top1_idx]); top2_p = float(probs[top2_idx])

    flip_reason = "kept_raw"
    # Only consider a flip if raw confidence was low
    if raw_top < RAW_MIN_KEEP:
        # need a strong alternative AND enough gap AND region support
        if (
            top2_p >= ALT_MIN
            and (top2_p - top1_p) >= GAP_MIN
            and roi_bbox is not None
            and _region_supports(top2_lbl, roi_bbox, H, W)
        ):
            # don’t flip to meningioma unless it actually touches the edge
            if not (top2_lbl == "meningioma" and not _edge_contact(roi_bbox, H, W)):
                adjusted_label = top2_lbl
                flip_reason = f"flip_to_{top2_lbl}_alt>=ALT_MIN_gap>=GAP_MIN_region_ok"

    # If pituitary was raw top-1 and ROI looks sellar, never flip away
    ...
    if len(order) >= 2:
        ...
        if margin < 0.12:
            dbg_lines.append(
                f"Uncertain: top-2 classes close ({LABELS[order[0]]} vs {LABELS[order[1]]}, Δ={margin:.3f})"
                 f"SegAware info: raw={raw_label}({raw_top:.3f}) → adjusted={adjusted_label} | "
    f" alt=({top2_lbl},{top2_p:.3f}) | reason={flip_reason}"
            )
    ...
    dbg_lines.append(f"SegAware info: raw={raw_label}({raw_top:.3f}) → adjusted={adjusted_label} | reason={flip_reason}")
...
with gr.Blocks(title="BRISC2025 Tumor Classifier (Clean + Seg-aware)") as demo:
    ...
    with gr.Row():
        img_in = gr.Image(
            type="pil",
            label="Upload BRISC-style MRI slice (JPG/PNG)",
        )
